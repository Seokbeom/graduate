...
from keras.layers import LSTM, Dropout, Bidirectional, Masking, Lambda ,Dense, RepeatVector, TimeDistributed
...
def main(T,Q,A): # 코드이해 30%
    ...
    # define model
    model = Sequential()
    model.add(Dropout(0.2, input_shape=(max_step, embedded_dim))) ##이거할까 #embedin layer를 추가할까
    model.add(Masking(mask_value=0, input_shape=(max_step, embedded_dim)))
    model.add(Bidirectional(LSTM(units=unit, input_shape=(max_step, embedded_dim)), merge_mode='sum'))  #
    model.add(RepeatVector(max_step))
    model.add(LSTM(unit, return_sequences=True))
    model.add(TimeDistributed(Dense(embedded_dim, activation='tanh')))
    ...
    #model.add(Dense(units=unit, input_shape=(max_step, embedded_dim), activation='tanh'))# model.add(TimeDistributed( Dense(units=embedded_dim, input_shape=(max_step, embedded_dim), activation='softmax')))
    model.summary()
