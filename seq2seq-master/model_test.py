...
def main(data_to_read): # 코드이해 30%
    ...
    #from seq2seq.cells  import AttentionDecoderCell


    global max_step
    print(n_features)
    '''
    a= AttentionSeq2Seq(input_shape = (max_step, n_features), hidden_dim=unit, output_length=max_step, output_dim=n_features, depth=1, bidirectional=True)
    r = Input(shape=( unit, max_step, n_features))
    rr = Input(shape=(unit, max_step, n_features))
   # rr= Dense(units=n_features)
    #model2 = Model(input=[max_step, n_features], output=n_features)
    R1 = RecurrentModel(r, output = rr) # Recurrent = LSTM으로??
    R2= RecurrentSequential(R1)
    ##X = np.zeros((len( data_in_lang[0]), max_step, n_features), dtype=np.bool)
    #X= list(X)
    #print(type(X))
    #R3 = R2.build(X)
   # b =  AttentionDecoderCell()
    #c= b.build_model(b,(max_step, n_features))
    model = load_model('movie_dialogue_10_Extracted_Lemmatizedone_hot.h5', custom_objects={
        'AttentionSeq2Seq': a,
        'RecurrentSequential': R2,
        '_OptionalInputPlaceHolder': _OptionalInputPlaceHolder,
        'AttentionDecoderCell':AttentionDecoderCell(units = unit)
        })
    '''

    model = load_model('movie_dialogue_10_Extracted_Lemmatizedone_hot.h5', custom_objects={'AttentionDecoder': AttentionDecoder})
